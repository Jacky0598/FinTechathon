# 文件: app.py (前端 - 真实API调用版)
import streamlit as st
import time
import pandas as pd
import matplotlib.pyplot as plt
import numpy as np
import re
import random
import requests  # <-- ‼️ 必须导入这个新包

# -----------------------------------------------------------------
# 页面配置 (Page Config)
# -----------------------------------------------------------------
# 设置页面标题、图标和布局
st.set_page_config(
    page_title="Holistica Quant",
    page_icon="🤖",
    layout="wide"
)


# -----------------------------------------------------------------
# ‼️‼️【真实版】后端API调用 ‼️‼️
# -----------------------------------------------------------------
def call_holistica_backend(query: str):
    """
    【真实版】
    调用在 http://127.0.0.1:8000 运行的 FastAPI 后端。
    """

    # 你的FastAPI后端运行的地址 (main.py 运行的地方)
    BACKEND_URL = "http://127.0.0.1:8000/analyze"

    with st.spinner(f"🤖 Holistica Agent 正在分析中... (正在调用真实后端)"):
        try:
            # 1. 将用户的提问作为JSON发送到后端
            # 设置一个超时时间（例如10秒）
            response = requests.post(BACKEND_URL, json={"query": query}, timeout=30)

            # 2. 检查后端是否成功
            if response.status_code == 200:
                # 3. 从后端获取JSON响应并返回
                # 它的结构是 {"report": "...", "data": {...}}
                return response.json()
            else:
                # 如果后端出错 (比如500错误)
                st.error(f"后端API错误 (状态码: {response.status_code})")
                st.error(f"错误详情: {response.text}")
                return {"report": "后端分析失败，请检查后端服务器。", "data": {}}

        except requests.exceptions.ConnectionError:
            # 如果后端服务器没有运行
            st.error("❌ 无法连接到后端服务器！")
            st.error("请确认你的FastAPI后端正在 http://127.0.0.1:8000 运行。")
            st.info(
                "如何运行后端：\n 1. 打开一个新的终端 \n 2. `cd backend` \n 3. `pip install -r requirements_be.txt` \n 4. `uvicorn main:app --reload`")
            return {"report": "连接失败。", "data": {}}
        except requests.exceptions.Timeout:
            st.error("❌ 后端请求超时！")
            st.error("后端分析时间过长，已中断。请检查后端逻辑或延长超时时间。")
            return {"report": "请求超时。", "data": {}}


# -----------------------------------------------------------------
# 侧边栏导航 (Sidebar Navigation)
# -----------------------------------------------------------------
st.sidebar.title("🤖 Holistica Quant")
st.sidebar.markdown("---")
page = st.sidebar.radio(
    "功能模块 (Modules)",
    [
        "欢迎 (Welcome)",
        "模块一：金融科技学习 (FinTech Learning)",
        "模块二：投研实验室 (Investment Lab)",
        "模块三：AI伴随分析 (AI Companion)",
        "模块四：全球趋势与新闻 (News & Trends)"
    ],
    captions=[
        "项目介绍",
        "AI互动讲师",
        "虚拟投资与回测",
        "自然语言驱动分析",
        "财报与黑天鹅追踪"
    ]
)
st.sidebar.markdown("---")
st.sidebar.info("微众银行金融科技大赛")

# -----------------------------------------------------------------
# 页面内容 (Page Content)
# -----------------------------------------------------------------

# -----------------------------------------------------------------
# 页面内容 (Page Content)
# -----------------------------------------------------------------

if page == "欢迎 (Welcome)":
    st.title("欢迎来到 Holistica Quant 🤖")
    st.subheader("一个端到端的AI驱动的深度洞见与研究分析 | 虚拟量化投资实验室 [cite: 271]")
    st.caption("Al-powered Insight Investment Lab [cite: 272]")

    st.markdown("---")

    # 根据 TPRD.pdf 的内容更新 [cite: 303, 313, 314]
    st.subheader("🎯 产品定位与核心价值")
    st.markdown("""
    我们是一个面向金融机构与高校的AI金融教育技术解决方案 [cite: 303]。

    * **核心价值:** 降低金融科技学习门槛，提供AI原生的智能金融学习与实践平台 [cite: 313]。
    * **市场价值:** 为微众银行提供一个“AI人才孵化+创新实验平台” [cite: 314]。
    """)

    # 根据 TEAM.pdf 的内容更新 [cite: 775, 777-782]
    st.subheader("👥 团队成员")
    st.markdown("""
    * **产品 (Product):** Caroline, Iris [cite: 777, 778]
    * **研发 (R&D):** Jess, Jacky [cite: 779, 780]
    * **辅助 (Support):** Maxen [cite: 781, 782]
    """)

    st.markdown("---")

    # 解决的痛点
    st.subheader("📌 解决的痛点")
    st.markdown("""
    1.  **金融学习门槛高:** 金融知识体系庞大、数据和工具碎片化 [cite: 282]。
    2.  **产学研脱节:** 高校课程与行业企业缺乏实践项目，教育与实践脱节 [cite: 289, 290]。
    3.  **毕业生技能脱节:** 缺乏“项目式实操经验”，理论与实际差距大 [cite: 294, 296]。
    """)

    st.info("后端服务器 (FastAPI) 状态：请在“模块三”中测试API连接。")


elif page == "模块一：金融科技学习 (FinTech Learning)":
    st.title("📚 模块一：金融科技学习")
    st.info("此模块让AI充当“互动讲师+实验指导”，帮助用户理解核心机制。")
    topic = st.selectbox(
        "选择一个学习主题：",
        ["支付 (Payment)", "储蓄与借贷 (Lending)", "投资管理 (Investment)", "市场预测 (Forecasting)"]
    )
    st.write(f"你选择了: **{topic}**")
    st.chat_input("请向AI讲师提问 (例如：什么是P2P？)")


elif page == "模块二：投研实验室 (Investment Lab)":
    st.title("🔬 模块二：投研实验室")
    st.info("此模块提供虚拟AI投研实验室，用于构建和测试投资策略。")
    st.subheader("实验案例：特斯拉(TSLA)估值是否合理？")
    if st.button("开始基本面分析实验"):
        with st.spinner("AI正在提取Tesla财报 (2020-2025) 并与Ford、BYD对比..."):
            time.sleep(3)
            st.success("分析完成！")
            st.markdown("#### 投研报告：《Fundamental Valuation of Tesla (2020-2025)》")
            st.write("AI分析结论：Hold (模拟)")


elif page == "模块三：AI伴随分析 (AI Companion)":
    st.title("🤝 模块三：AI伴随分析 (AI Companion)")
    st.info("一个“懂金融科技”的智能AI助手，通过自然语言与您伴随分析、解释、生成报告。")

    default_query = "分析一下 特斯拉"
    query = st.text_area("请输入你的分析请求:", value=default_query, height=100)

    if st.button("🚀 开始分析"):
        if query:
            # 1. ‼️‼️ 调用真实的后端 ‼️‼️
            result = call_holistica_backend(query)

            # 2. 显示后端返回的报告
            st.markdown("---")
            st.markdown(result["report"])

            # 3. ‼️‼️ 解析并显示后端返回的图表数据 ‼️‼️
            # 检查 'data' 键是否存在，并且不为空，并且 'price_history' 键在 'data' 内部
            if "data" in result and result["data"] and "price_history" in result["data"]:
                st.subheader("相关性走势图表 (来自FastAPI的数据)")

                # 将后端传来的列表数据转为 Pandas Series 来绘图
                price_data = result["data"]["price_history"]

                if price_data:  # 确保数据列表不为空
                    chart_data_series = pd.Series(price_data)

                    fig, ax = plt.subplots()
                    chart_data_series.plot(ax=ax, grid=True, title=f"模拟价格走势 (来自后端)")
                    ax.set_ylabel("Price (from Backend)")
                    plt.tight_layout()
                    st.pyplot(fig)
                else:
                    st.write("后端返回了空的图表数据。")
            else:
                st.write("后端未返回有效的图表数据。")
        else:
            st.error("请输入分析请求。")


elif page == "模块四：全球趋势与新闻 (News & Trends)":
    st.title("📰 模块四：全球趋势与新闻")
    st.info("实时追踪全球金融科技趋势、最新财报与黑天鹅事件。")
    st.subheader("全球财报追踪与AI解读 (模拟)")
    st.markdown("""
    * **苹果 (AAPL)**: Q4 财报发布，营收超预期。
    * **英伟达 (NVDA)**: AI芯片需求持续强劲，指引上调。
    """)
    st.subheader("黑天鹅专区 (模拟)")
    st.warning("突发：某大型航运公司宣布因不可抗力暂停红海航线。")